...
class ProductBloc {
  ...
  Stream<List<Product>> productByVendorId(String vendorId) =>
      db.fetchProductByVend0rId(vendorId);
  Future<Product> fetchProduct(productId) => db.fetchProduct(productId);
  ...
  Future<void> saveProduct() async {
    ...
    return db.setProduct(product).then((value) => _productSaved.sink.add(true),
        onError: (error) => _productSaved.sink.add(false));
  }
  ...
  final validateUnitPrice = StreamTransformer<String, double>.fromHandlers(
    ...
    if (unitPrice != null) {
      try {
        ...
        sink.addError("Try with a valid unit price");
      }
    }
  });
  ...
  final validateAvailableUnits = StreamTransformer<String, int>.fromHandlers(
    ...
    if (availableUnits != null) {
      try {
        ...
        sink.addError("Try with valid available units");
      }
    }
  });
